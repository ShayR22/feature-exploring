...
class CommonHeaderFinder:
    def __init__(self, source_dir: str, most_common_amount_printed=10):
        header_counter = Counter()
        cpp_files = Path(source_dir).rglob('*.cpp')
        logging.debug(f"{cpp_files=}")
        for file in cpp_files:
            rhc = RecursiveHeaderCount()
            headers = rhc.get_headers_recusrivly(file)
            logging.info(f"{file=} {len(headers)=}")
            header_counter.update(headers)

        logging.info("===============================")
        for header, count in header_counter.most_common(most_common_amount_printed):
...
if __name__ == "__main__":
    ...
    chf = CommonHeaderFinder(directory)
